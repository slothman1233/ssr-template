...
export const nunRenderString = (str: string, optins: any) => {
  ...
  const html = htmlMinifier.minify(nunjucksEVN.renderString(str, Object.assign({}, optins)), {
    collapseWhitespace: true,
  });
 ...
 */
export const nunRenderMacroString = async (
  filepath: string,
  name: string,
  options: any,
): Promise<string> => {
  ...
  // @ts-ignore
  const reg = /\{%\s*?macro\s*?([^(].*?)\(.*?%\}(.*?)\{%\s*?endmacro\s*?%\}/gis;
  const paths = path.resolve(__dirname, '..', '..', 'views/shared/sharedView/macro', filepath);
  const isfile = await isFile(paths);

  if (!isfile) {
    return '';
  }

  const getstring = (await readFile(paths)).toString();
  ...
  const filep = paths.slice(0, paths.lastIndexOf('\\'));

  hreadstring = hreadstring.replace(/['"](.*?)['"]/gis, function ($1, $2): string {
    const p = path.resolve(filep, $2);
    const pathArg = p.split('\\').filter(function (s) {
      return s && s.trim();
    });
